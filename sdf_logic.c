...
static inline void compare(int i, int ni, float ox, float oy, float *dx,
                           float *dy, uint32_t *seeds, float *add_dist) {
  float nx = dx[ni] + ox;
  float ny = dy[ni] + oy;
...
static inline float remap_clamped(float val, float in_min, float in_max,
...
DLLEXPORT void compute_sdf(int w, int h, uint8_t *pixels, float max_range,
                           uint8_t* out_pixels) {
  int total = w * h;
    ...
    for (int x = 0; x < w; x++) {
      ...
      if (y > 0) { // Top
        ...
        if (x > 0) { // Top-Left
          ...
          compare(i, i - w - 1, 1.0f, 1.0f, dx_in, dy_in, seeds_in,
                  add_dist_in);
        }
      }
    }
  ...
  // 3. RENDER AND OUTPUT
  for (int i = 0; i < total; i++) {
    float d_out = sqrtf(dx[i] * dx[i] + dy[i] * dy[i]) + add_dist[seeds[i]];
    float d_in = sqrtf(dx_in[i] * dx_in[i] + dy_in[i] * dy_in[i]) +
                 add_dist_in[seeds_in[i]];
    float val;

    if (pixels[i * 4] > 127) {
      val = remap_clamped(d_in, 1.0f, max_range + 1, 0.5f, 1.0f);
    } else {
      val = remap_clamped(d_out, 0.0f, max_range + 1, 0.5f, 0.0f);
    }

    uint8_t gray = (uint8_t)(val * 255.0f);
    int o = i * 4;
    out_pixels[o] = out_pixels[o + 1] = out_pixels[o + 2] = gray; // BGR
    out_pixels[o + 3] = 255;                                      // Alpha
  }
